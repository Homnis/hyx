...
class end():
    ...
    def __del__(self):
        with open(r"E:\hyx\core\聊天软件的实现_v1.0\client.txt", "rb") as isNone:  # 用文件保存用户名
            reading = isNone.read()
        ...
        with open(r"E:\hyx\core\聊天软件的实现_v1.0\client.txt", "wb") as d:
            d.write(names)
        labelSet = labelName()
        # print(labelSet)
        if labelSet==set():
...
def resetServer():
    with open(r"E:\hyx\core\聊天软件的实现_v1.0\server.txt", "w") as cover:
        cover.truncate()
    ...
    while True:
        try:
            ...
            if labelSet is not None:
                ...
                delClient=now_clients-labelSet
                # print(delClient)
                if delClient!=set():
                    now_clients=now_clients-delClient
                    users.set("users:" + '\n')
                    for i in now_clients:
                        users.set(users.get() + i + '\n')
...
if __name__ == '__main__':
    ...
    # 客户端的启动也需要独立线程
    nowClient = getNowClient()
    client = threading_local.Thread(target=chatClient.core,args=(txt_msgsend,txt_msglist,nowClient,))
    client.start()
    '''
