...
def startServer():
    open(r"E:\hyx\core\聊天软件的实现\server.txt", 'a')
    with open(r"E:\hyx\core\聊天软件的实现\server.txt", "rb") as isNone:  # 通过server文件是否为空来判断服务端是否已在运行
        reading = isNone.read()
        # print(reading)
        if reading == b'':
            with open(r"E:\hyx\core\聊天软件的实现\server.txt", "w") as f:
                f.write("did")
...
def usr_login():
    ...
    psw = var_usr_pwd.get()
    open(r"E:\hyx\core\聊天软件的实现\reg.txt", 'a')
    with open(r"E:\hyx\core\聊天软件的实现\reg.txt", "rb") as isNone:  # 判断是否为空，不为空的话重新建立字典，代替原有的
        reading = isNone.read()
        # print(reading)
        if reading != b'':
            with open(r"E:\hyx\core\聊天软件的实现\reg.txt", "rb") as isTrue:
...
def createName(name):
    ...
    setNowClient(name)
    # print(now_client)
    open(r"E:\hyx\core\聊天软件的实现\client.txt", 'a')
    with open(r"E:\hyx\core\聊天软件的实现\client.txt", "rb") as isNone:  # 用文件保存用户名为列表
        reading = isNone.read()
        # now_client.add("x")
        now_client.add(name)
        ...
        if reading != b'':
            ...
            y.add(name)
            y = pickle.dumps(y)
            with open(r"E:\hyx\core\聊天软件的实现\client.txt", "wb") as cover:
                cover.write(y)
        else:
            with open(r"E:\hyx\core\聊天软件的实现\client.txt", "wb") as creat:
                creat.write(x)
        ...
        if psw_confirm != psw_get:
            ...
            if reading != b'':
                ...
                if getNewName in y.keys():
                    ...
                    y = pickle.dumps(y)
                    with open(r"E:\hyx\core\聊天软件的实现\reg.txt", "bw") as cover:
                        cover.write(y)
                    tk.messagebox.showinfo('Welcome', '您已经注册成功！')
                    window_sign_up.destroy()
            else:
                with open(r"E:\hyx\core\聊天软件的实现\reg.txt", "bw") as userReg:
                    userReg.write(x)
                tk.messagebox.showinfo('Welcome', '您已经注册成功！')
                window_sign_up.destroy()
